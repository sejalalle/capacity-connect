...
// ── AI Explanation helpers (§ Sanctioned AI slots) ───────────────────────────
// These functions produce readable text for human context. They never write
// competency records, update status, decide assignments, or substitute for human
// review. All are logged via aiAssist → P3AIRequestMetadata for audit visibility.

export async function explainGap(actor, gapRow) {
  const input = {
    competencyName: gapRow.competency?.name || "",
    domain: gapRow.competency?.domain || "",
    requiredLevel: gapRow.requiredLevel,
    demonstratedLevel: gapRow.demonstratedLevel ?? null,
    evidenceStatus: gapRow.evidenceStatus || "NOT_ASSESSED",
    category: gapRow.category || "",
    sourceReferenceIds: [String(gapRow.competency?._id || "")],
  };
  return aiAssist(actor, "SKILL_GAP_EXPLANATION", input);
}

export async function explainCourseRecommendation(actor, gapRow, course) {
  const input = {
    competencyName: gapRow.competency?.name || "",
    requiredLevel: gapRow.requiredLevel,
    demonstratedLevel: gapRow.demonstratedLevel ?? null,
    ...
    sourceReferenceIds: [String(course._id || "")],
  };
  return aiAssist(actor, "COURSE_RECOMMENDATION_EXPLANATION", input);
}

export async function explainTrainerMatch(actor, recommendation) {
  const input = {
    trainerName: recommendation.trainer?.name || "",
    status: recommendation.status,
    totalPoints: recommendation.totalPoints ?? null,
    reviewedLevel: recommendation.supportingFacts?.reviewedLevel ?? null,
    requiredLevel: recommendation.supportingFacts?.requiredLevel ?? null,
    factSummary: (recommendation.factors || [])
      .slice(0, 5)
      .map((f) => `${f.key}:${f.contribution}/${f.weight}`)
      .join("; "),
    sourceReferenceIds: [String(recommendation.trainer?._id || "")],
  };
  return aiAssist(actor, "TRAINER_MATCH_EXPLANATION", input);
}

export async function summarizeEvidence(actor, evidence) {
  const input = {
    evidenceType: evidence.evidenceType,
    description: (evidence.description || "").slice(0, 1000),
    claimCount: evidence.claimedCompetencies?.length || 0,
    competencies: (evidence.claimedCompetencies || [])
      .map((c) => c.competency?.name || "")
      .filter(Boolean)
      .slice(0, 5)
      .join(", "),
    status: evidence.status,
    sourceReferenceIds: [String(evidence._id || "")],
  };
  return aiAssist(actor, "EVIDENCE_SUMMARIZATION", input);
}

export async function summarizeFeedback(actor, feedbackItems) {
  const items = (Array.isArray(feedbackItems) ? feedbackItems : []).slice(
    0,
    50,
  );
  const input = {
    ...
    averageRating:
      items.length > 0
        ? (items.reduce((s, x) => s + (x.rating || 0), 0) / items.length).toFixed(1)
        : null,
    sourceReferenceIds: [],
  };
  return aiAssist(actor, "FEEDBACK_SUMMARIZATION", input);
}

export async function summarizeTttCandidate(actor, data) {
  const input = {
    candidateName: data.candidate?.name || "",
    programName: data.program?.title || "",
    competencyName: data.competency?.name || "",
    targetLevel: data.nomination?.targetLevel || 3,
    practiceTitle: data.practice?.sessionTitle || "",
    practiceSubmission: (
      data.practice?.teachingPlan ||
      data.practice?.materialsDescription ||
      ""
    ).slice(0, 500),
    evaluationOutcome: data.evaluation?.outcome || "",
    sourceReferenceIds: [String(data.nomination?._id || "")],
  };
  return aiAssist(actor, "TTT_CANDIDATE_SUMMARIZATION", input);
}
