...
test("AI explanation and summarization endpoints produce structured explanations without altering competency", async () => {
  process.env.AI_ENABLED = "true";
  process.env.AI_PROVIDER = "mock";

  // 1. Skill extraction rejection
  let response = await request(app)
    .post("/api/part3/ai/skill-extraction")
    .set(auth(trainee))
    .send({
      sourceReferenceId: "synthetic-cv-reject",
      sourceText: "Synthetic satellite cloud tracking experience.",
    })
    .expect(200);
  const rejectReqId = response.body.data.requestId;
  await request(app)
    .post(`/api/part3/ai/skill-extraction/${rejectReqId}/accept`)
    .set(auth(trainee))
    .send({ tag: "Satellite cloud tracking", action: "REJECTED" })
    .expect(200);
  const userAfterReject = await User.findById(trainee._id);
  assert.ok(!userAfterReject.skills.includes("Satellite cloud tracking"));
  const meta = await P3.P3AIRequestMetadata.findOne({ requestId: rejectReqId });
  assert.equal(meta.humanAction, "REJECTED");

  // 2. Skill gap explanation
  response = await request(app)
    .post("/api/part3/ai/explain-gap")
    .set(auth(trainee))
    .send({
      competencyId: p2.competencies[0]._id,
      requiredLevel: 3,
    })
    .expect(200);
  assert.ok(response.body.data.explanation.length > 0);
  assert.ok(response.body.data.disclaimer.includes("human review"));

  // 3. Course recommendation explanation
  response = await request(app)
    .post("/api/part3/ai/explain-course")
    .set(auth(trainee))
    .send({
      competencyId: p2.competencies[0]._id,
      requiredLevel: 3,
      courseId: p3.batch.course,
    })
    .expect(200);
  assert.ok(response.body.data.explanation.length > 0);

  // 4. Trainer match explanation
  const session0 = p3.batch.sessions?.[0] || { _id: new mongoose.Types.ObjectId() };
  response = await request(app)
    .post("/api/part3/ai/explain-trainer-match")
    .set(auth(trainee))
    .send({
      batchId: p3.batch._id,
      sessionId: session0._id,
    })
    .expect(200);
  assert.ok(response.body.data.justification.length > 0);

  // 5. Evidence summarization
  response = await request(app)
    .post("/api/part3/ai/explain-evidence")
    .set(auth(trainee))
    .send({
      evidenceId: evidence._id,
    })
    .expect(200);
  assert.ok(response.body.data.summary.length > 0);
  assert.ok(Array.isArray(response.body.data.keyPoints));

  // 6. Feedback summarization
  response = await request(app)
    .post("/api/part3/ai/summarize-feedback")
    .set(auth(reviewer))
    .send({
      batchId: p3.batch._id,
    })
    .expect(200);
  assert.ok(response.body.data.themes.length > 0);

  // 7. TTT candidate summarization
  response = await request(app)
    .post("/api/part3/ai/summarize-ttt-candidate")
    .set(auth(admin))
    .send({
      nominationId: p3.tttReady._id,
    })
    .expect(200);
  assert.ok(response.body.data.candidateBlurb.length > 0);
  assert.ok(Array.isArray(response.body.data.teachingStrengths));

  // Verify that none of these explanation calls created competency decisions or modified records
  const decisionsCount = await P3.P3CompetencyDecision.countDocuments();
  assert.equal(
